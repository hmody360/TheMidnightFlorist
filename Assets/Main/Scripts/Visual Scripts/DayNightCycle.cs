...
public class DayNightCycle : MonoBehaviour
{
    ...
    [Header("VFX")]
    private DayNightVFXController _VFXController;
    ...
    private void Awake()
    {
        _VFXController = GetComponent<DayNightVFXController>();
    }
    ...
    private void UpdateSkybox()
    {
        ...
        if (_currentSkyBox != targetSkyBox)
        {
            ...
            {
                _VFXController.HideNightParticles();
                _VFXController.ShowDayParticles();
                _audioManager.ChangeAmbienceSounds(1);
            }else if (_currentSkyBox == _skyboxList[2])
            {
                _VFXController.HideDayParticles();
                _audioManager.ChangeAmbienceSounds(2);
            }else if (_currentSkyBox == _skyboxList[3])
            {
                _VFXController.ShowNightParticles();
            }
        }
    }
